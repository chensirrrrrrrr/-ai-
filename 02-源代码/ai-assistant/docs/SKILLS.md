...
> **pytest 472/472（91%）/ smoke 149/149 / E2E 37/37**；再补工具层分支 30 条 → **502/502（93%）**；行级隔离修复 +1 条 → **503/503（93%）**；限流+监控探针 +9 条 → **512/512（93%）**。
...
**再收口 `agent_tools.py`（78%→100%，30 条）→ 502/502（93%）**；行级隔离 +1 → 503/503（93%）**；限流+监控 +9 → 512/512（93%）**。
